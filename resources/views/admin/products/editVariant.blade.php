...
<div class="container mt-4">
    ...
    @endif
    <h2 class="mb-4">Sửa Sản Phẩm biến thể</h2>
    <form action="{{ route('admin.products.variants.update', [ $product->id,$variant->id]) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3 col-md-6">
            ...
            @enderror" onchange="toggleSizeBox()" >
                ...
                @foreach ($sizeMls as $sizeMl)
                    <option value="{{ $sizeMl->id }}" {{ old('size_ml_id', $variant->size_ml_id) == $sizeMl->id ? 'selected' : '' }}>
                        {{ $sizeMl->size_ml_name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3 col-md-6">
            ...
            @enderror" disabled>
                ...
                @foreach ($sizeBoxes as $sizeBox)
                    <option value="{{ $sizeBox->id }}" {{ old('size_box_id', $variant->size_box_id) == $sizeBox->id ? 'selected' : '' }}>
        ...
        <div class="mb-3 col-md-6">
            ...
            <input type="number" name="price" class="form-control @error('price') is-invalid @enderror" value="{{ old('price', $variant->price) }}" min="0" >
            @error('price') <div class="text-danger">{{ $message }}</div> @enderror
        </div>
        <div class="mb-3 col-md-6">
            <label class="form-label ">Giá khuyến mãi</label>
            <input type="number" name="promotional_price" class="form-control @error('promotional_price', $variant->promotional_price) is-invalid @enderror" value="{{ old('promotional_price') }}" min="0">
            @error('promotional_price') <div class="text-danger">{{ $message }}</div> @enderror
        </div>

        <div class="mb-3 col-md-12">
            <label for="" class="form-label ">Số lượng</label>
            <input type="number" name="stock" class="form-control @error('stock') is-invalid @enderror " value="{{ old('stock', $variant->stock) }}" min="0" >
        ...
        <button type="submit" class="btn btn-primary">Sửa biến thể</button>
